	...
	class DF_Megamenu {
	  ...
	  //   }
		...
		function df_megamenu_load_script() {
			...
			#wp_enqueue_script( 'df_megamenu_js', get_template_directory_uri().'/inc/js/megamenu.js', array('jquery'), '1.0', true );
			#wp_localize_script( 'df_megamenu_js', 'ajax_megamenu', array( 'ajaxurl' => admin_url( 'admin-ajax.php' ) ) );
			/**
			 * temporary load bootstrap
			 */
		}
			...
			foreach ($items as &$item) {
				...
				if($megadropdown_menu_cat != ''){
					...
					if(sizeof($sub_cat) == 0){ // if cat has no sub-cat with posts
						...
						$new_item->title .= '<div class="df-loading hidden df-loading-'.esc_attr( $megadropdown_menu_cat ).'-'.esc_attr( $no_item ).'">LOADING</div>';
						...
						if ( false == get_theme_mod( 't_o_all', false ) ) { $t_o_all = esc_html__("All", "magazine-plug");  } else { $t_o_all = get_theme_mod( 't_o_all' ); }
						// load sub categories as navigation
						$new_item->title .= '<div class="col-md-3 df-subcat-stack">';
						$new_item->title .= '<ul class="nave nav-stacked df-megamenu-nav-sub" id="megamenu-'.esc_attr( $megadropdown_menu_cat ).'-'.esc_attr( $no_item ).'">';
						$new_item->title .= '<li class="active">';
						$new_item->title .= '<a data-target="#df-pane-'.esc_attr( $megadropdown_menu_cat ).'-'.esc_attr( $no_item ).'" href="'.get_category_link( $megadropdown_menu_cat ).'" class="mt-sub-cat-a">'.$t_o_all.'</a>';
						$new_item->title .= '</li>';
						foreach($sub_cat as $sc){

							$new_item->title .= '<li class="">';
								$new_item->title .= '<a data-target="#df-pane-'.esc_attr( $sc->cat_ID ).'-'.esc_attr( $no_item ).'" href="'.get_category_link( $sc->cat_ID ).'" class="mt-sub-cat-a">'.$sc->cat_name.'</a>';
							$new_item->title .= '</li>';
						}
						$new_item->title .= '</ul>';
						$new_item->title .= '</div>';
	    				...
	    				$stylefirst = 'pointer-events: none; cursor: default; color: #ccc';
			       		$new_item->title .= '<div class="row">
			       							<div class="row_next_prev hidden row_next_prev-'.esc_attr($megadropdown_menu_cat).'-'.esc_attr($no_item).'">
					       						<div class="" style="">
						...
						foreach ($sub_cat as $countersc) {
							...
							if($this->df_get_found_posts() > 4){
		    					$stylefirst = 'pointer-events: none; cursor: default; color: #ccc';
				       			$new_item->title .= '<div class="row">
				       								<div class="row_next_prev hidden row_next_prev-'.esc_attr($countersc->cat_ID).'-'.esc_attr($no_item).'">
			       										<div class="" style="">
			           		...
			           		$new_item->title .= '<div class="row-inner">';// open tag for row block inner megamenu
							// open tag for inner megamenu
							$new_item->title .= '<div class="df-loading hidden df-loading-'.esc_attr( $countersc->cat_ID  ).'-'.esc_attr( $no_item ).'">LOADING</div>';
	    ...
	    function df_render_inner($posts, $cat_id, $no_item, $has_sub_cat){
	    	...
	    	if(!empty($posts)) {
	    		...
	    		if($has_sub_cat == 'false'){
	    			...
	    			$buff .= '<div class="row mega-post-wrap megamenu-grid-container-'.esc_attr($cat_id).'-'.esc_attr($no_item).'">';
						foreach ($posts as $post) {
		    			$buff .= '<div class="megamenu-span mega-5">';
							$buff .= '<div class="mega-post-in">';
	    ...
	    function image_post($post){
	    	$thumbs='';
				if( has_post_thumbnail( $post->ID) ){
								$thumbs .='<div class="mt-megamenu-img poster-image mt-radius"><div class="mt-post-image"><img alt="'.$post->post_title.'" class="lazy" src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7"  data-src="'. get_the_post_thumbnail_url($post->ID,'medium').'" width="550" height="550" /></div></div>';
				}else{
	     		$thumbs = '';
	     	}
	    	return $thumbs;
		 ...
		 * do_action('df_generate_megamenu')
		 */
		static function df_call_megamenu(){
			do_action('df_generate_megamenu');
		}
	}
